...
class _ForYouPageState extends State<ForYouPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _clearForYouData(); // Clear data on init for fresh state
  ...
  void _clearForYouData() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ...
        ticket.clearData();
        log('🧹 Cleared ForYouPage data and all TicketServices data');
      }
    });
  }
  ...
  void _performSearch(String orderNumber) {
    ...
    try {
      final ticket = Provider.of<TicketServices>(context, listen: false);
      ticket.forYouTicketSearch(orderNumber.trim());
      log('🔍 Searching for 4uwin-5 order number: $orderNumber');
  ...
  Future<void> handleBulkPayment(TicketServices ticket, List<Map<String, dynamic>> winningTickets) async {
    ...
    final orderNumber = ticket.foryouTicketData?['order_number']?.toString();
    if (orderNumber == null) {
  ...
  List<Map<String, dynamic>> _findWinningTickets(TicketServices ticket) {
    if (ticket.foryouTicketData == null || ticket.foryouTicketData!['validTickets'] == null) return [];

    final tickets = ticket.foryouTicketData!['validTickets'] as List<dynamic>;
    ...
    for (var ticketData in tickets) {
      ...
      if (matchedPrice > 0) {
        log(
          '✅ Found winning ticket with ID: ${ticketData['id']}, matched_price: $matchedPrice, matched_numbers: ${ticketData['matched_numbers']}',
        );
  ...
  @override
  void dispose() {
    _clearForYouData(); // Clear data when leaving
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        _clearForYouData(); // Clear data on back navigation
        return true;
      },
      child: Scaffold(
        drawer: AppDrawer(),
        appBar: AppBar(title: AppText('4uwin-5', fontSize: 16.sp, fontWeight: FontWeight.bold)),
        body: Stack(
          children: [
            Padding(
                ...
                builder: (context, ticket, child) {
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      Center(child: AppText('4uwin-5 Ticket Search', fontSize: 20.sp)),
                                        ...
                                        Container(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              _buildDetailRow(
                                                'Order Number:',
                                                ticket.foryouTicketData!['order_number'].toString(),
                                              ),
                                              _buildDetailRow(
                                                'Order Date:',
                      ...
                      if (ticket.foryouTicketData != null && winningTickets.isEmpty && !ticket.isLoading)
                        Expanded(
                          child: Center(
                            child: Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          child: AppText(
                                            '🎉 4uwin-5 Winner${invoiceData!['ticketCount'] > 1 ? 's' : ''}! 🎉',
                                            fontSize: 18.sp,
                                            fontWeight: FontWeight.bold,
                                            textAlign: TextAlign.center,
                                            color: Colors.green[800],
