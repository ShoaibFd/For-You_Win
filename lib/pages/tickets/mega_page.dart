...
class _MegaPageState extends State<MegaPage> with TickerProviderStateMixin {
  ...
  Future<void> handlePayment(TicketServices ticket, Map<String, dynamic> matchedTicket) async {
    ...
    invoiceData = {
      ...
      'status': ticket.megaTicketData!['status'],
  ...
  Future<void> handlePrint(Map<String, dynamic> matchedTicket) async {
    ...
    try {
      ...
      final file = File('${tempDir.path}/mega_ticket_${matchedTicket['id']}.pdf');
      ...
      if (mounted) {
        AppSnackbar.showSuccessSnackbar('Mega ticket receipt printed successfully!');
      }
    } catch (e) {
  ...
  String generateQRData(Map<String, dynamic> matchedTicket) {
    return 'TicketID:${matchedTicket['id']}|Product:${matchedTicket['product_name']}|DrawDate:${matchedTicket['draw_date']}|Prize:${matchedTicket['raffle_draw_prize']}';
  }
        ...
        build: (context) {
          return [
            ...
            pw.Container(
              ...
              child: pw.Column(
                children: [
                  invoiceRow('Ticket ID:', matchedTicket['id'].toString(), fontSize: 14),
                  invoiceRow('Product Name:', matchedTicket['product_name'].toString(), fontSize: 14),
                  invoiceRow('Candidate:', matchedTicket['candidate']['name'].toString(), fontSize: 14),
                  invoiceRow('Order Date:', matchedTicket['order_date'].toString().split('T').first, fontSize: 14),
                  invoiceRow('Draw Date:', matchedTicket['draw_date'].toString().split('T').first, fontSize: 14),
                  invoiceRow('Prize Amount:', 'AED ${matchedTicket['raffle_draw_prize']}', bold: true, fontSize: 16),
                ],
              ),
            ),
            pw.SizedBox(height: 20),
                        ...
                        child: SingleChildScrollView(
                          child: Column(
                            ...
                            children: [
                              ...
                              ...ticket.megaTicketData!['validTickets']?.map<Widget>((matched) {
                                    final ticketId = matched['id'].toString();
                                    final isPaid = matched['paid_by'] != null;

                                    formatDate(String rawDate) {
                                      return rawDate.split('T').first; // removes time part
                                    }
                                    ...
                                    final rows = [
                                      ...
                                      MapEntry('Draw Date', formatDate(matched['draw_date'].toString())),
                                      MapEntry('Order Date', formatDate(matched['order_date'].toString())),
                                    ];
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          child: Column(
                                            children: [
                                              infoRow('Ticket ID:', invoiceData!['ticket']['id'].toString()),
                                              Divider(),
                                              ...
                                              infoRow(
                                                'Order Date:',
                                                invoiceData!['ticket']['order_date'].toString().split('T').first,
                                              ),
                                              Divider(),
                                              infoRow(
                                                'Draw Date:',
                                                invoiceData!['ticket']['draw_date'].toString().split('T').first,
                                              ),
                                              Divider(),
                                              infoRow(
                                                'Prize Amount:',
                                                'AED ${invoiceData!['ticket']['raffle_draw_prize']}',
                                                isHighlighted: true,
                                              ),
                                            ],
                                          ),
...
Widget tableCell(String text, {bool isPaid = false}) {
  return Padding(
    padding: EdgeInsets.all(8.w),
    child: AppText(text, fontSize: 14.sp, color: isPaid ? Colors.green[700] : null),
  );
}
