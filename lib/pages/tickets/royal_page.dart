...
class _RoyalPageState extends State<RoyalPage> with TickerProviderStateMixin {
  // Controllers
  ...
  void initState() {
    ...
    _clearRoyalData(); // Clear data on init for fresh state
  ...
  void _clearRoyalData() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ...
        ticket.clearData();
        log('🧹 Cleared RoyalPage data and all TicketServices data');
      }
    });
  }
  ...
  void _performSearch(String orderNumber) {
    ...
    try {
      final ticket = Provider.of<TicketServices>(context, listen: false);
      ticket.royalTicketSearch(orderNumber.trim());
      log('🔍 Searching for Royal-6 order number: $orderNumber');
  ...
  Future<void> handleBulkPayment(TicketServices ticket, List<Map<String, dynamic>> winningTickets) async {
    ...
    final orderNumber = ticket.royalTicketData?['order_number']?.toString();
    if (orderNumber == null) {
    ...
    try {
      ...
      for (var matchedTicket in winningTickets) {
        final rawTicketId = matchedTicket['ticket_id'];
  ...
  List<Map<String, dynamic>> _findWinningTickets(TicketServices ticket) {
    if (ticket.royalTicketData == null || ticket.royalTicketData!['matched_tickets'] == null) return [];

    final tickets = ticket.royalTicketData!['matched_tickets'] as List<dynamic>;
    ...
    for (var ticketData in tickets) {
      ...
      if (matchedPrice > 0) {
        log(
          '✅ Found winning ticket with ID: ${ticketData['ticket_id']}, matched_price: $matchedPrice, matched_numbers: ${ticketData['matched_numbers']}',
        );
  ...
  String _extractCandidateName(Map<String, dynamic> ticket) {
    ...
    return candidate?.toString() ?? 'Unknown';
  }
  ...
  @override
  void dispose() {
    _clearRoyalData(); // Clear data when leaving
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        _clearRoyalData(); // Clear data on back navigation
        return true;
      },
      child: Scaffold(
        drawer: AppDrawer(),
                ...
                builder: (context, ticket, child) {
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      Center(child: AppText('Royal-6 Ticket Search', fontSize: 20.sp)),
                                        ...
                                        Container(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              _buildDetailRow('Order Number:', ticket.royalTicketData!['order_number']),
                                              _buildDetailRow(
                                                'Order Date:',
                                              ...
                                              // _buildDetailRow(
                                              //   'Matched Numbers:',
                                              //   (currentWinningTicket['matched_numbers'] as List<dynamic>?)?.join(
                                              //         ', ',
                                              //       ) ??
                                              //       'N/A',
                                              // ),
                      ...
                      if (ticket.royalTicketData != null && winningTickets.isEmpty && !ticket.isLoading)
                        Expanded(
                          child: Center(
                            child: Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          child: AppText(
                                            '🎉 Royal-6 Winner${invoiceData!['ticketCount'] > 1 ? 's' : ''}! 🎉',
                                            fontSize: 18.sp,
                                            fontWeight: FontWeight.bold,
                                            textAlign: TextAlign.center,
                                            color: Colors.green[800],
